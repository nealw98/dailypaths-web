...
(() => {
  ...
  document.querySelectorAll('.boundaries-guide .boundary-insert').forEach(insert => {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'visually-hidden boundary-open';
    button.textContent = `Enlarge: ${insert.querySelector('h3').textContent}`;
    button.setAttribute('aria-haspopup', 'dialog');
    button.setAttribute('aria-controls', 'boundary-reading-view');
    insert.prepend(button);
    insert.classList.add('boundary-insert--expandable');
    insert.addEventListener('click', event => {
      ...
      copy.querySelector('.boundary-open').remove();
      copy.classList.remove('boundary-insert--expandable');
      copy.querySelectorAll('[id]').forEach(element => element.removeAttribute('id'));
      copy.querySelector('h3').id = 'boundary-dialog-title';
      copy.setAttribute('aria-labelledby', 'boundary-dialog-title');
