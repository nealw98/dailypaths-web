...
export function renderSurrenderGuide(topic, featuredReadings, readings) {
  ...
  const rope = insert('Dropping the Rope', 'rope', `<p class="surrender-insert-deck">${inline(ropeSubtitle)}</p><div class="rope-columns">${['When I Keep Pulling', 'When I Drop the Rope'].map((heading, side) => `<section><h4 class="rope-column-title">${heading}</h4><ul>${ropeRows.map(row => `<li><h5>${inline(row[side * 2])}</h5><p>${inline(row[side * 2 + 1])}</p></li>`).join('')}</ul></section>`).join('')}</div><p class="surrender-insert-footer"><em>${inline(ropeFooter)}</em></p>`);
