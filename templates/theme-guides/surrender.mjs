...
export function renderSurrenderGuide(topic, featuredReadings, readings) {
  ...
  const ropeRows = rows('Dropping the Rope');
  const rope = '<div class="rope-frame">' + insert('Dropping the Rope', 'rope', `<p class="surrender-insert-deck">${inline(ropeSubtitle)}</p><div class="rope-columns">${['When I Keep Pulling', 'When I Drop the Rope'].map((heading, side) => `<section><h4 class="rope-column-title">${heading}</h4><ul>${ropeRows.map(row => `<li><h5>${inline(row[side * 2])}</h5><p>${inline(row[side * 2 + 1])}</p></li>`).join('')}</ul></section>`).join('')}</div><p class="surrender-insert-footer"><em>${inline(ropeFooter)}</em></p>`) + '</div>';

  const orbit = `<figure class="surrender-orbit" aria-label="When Life Revolves Around Theirs"><div class="surrender-orbit-art" data-reading-insert><h3 class="visually-hidden">When Life Revolves Around Theirs</h3><img src="${bp('/assets/guides/surrender/life-revolves-around-theirs.png')}" width="1536" height="1024" loading="lazy" alt="My sleep, mood, plans, friendships, and attention orbit around their life at the center." aria-describedby="orbit-transcript"></div><div class="visually-hidden" id="orbit-transcript"><h4>When Life Revolves Around Theirs</h4><p>At the center: <strong>Their life</strong></p><dl>${rows('When Life Revolves Around Theirs').map(([name, text]) => `<div><dt>${inline(name)}</dt><dd>${inline(text)}</dd></div>`).join('')}</dl><p><em>Their life becomes the center. Your own life waits.</em></p></div></figure>`;
  const body = [];
